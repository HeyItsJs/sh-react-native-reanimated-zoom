...
export function Zoom(props) {
    const { minimumZoomScale = 1, maximumZoomScale = 8, defaultZoomScale = 2.5, style: propStyle, onLayout, onZoomBegin, onZoomEnd, simultaneousGesture, } = props;
    ...
    const gesture = useMemo(() => {
        ...
        const doubleTap = Gesture.Tap()
            ...
            if (scale.value !== 1) {
                ...
                translationX.value = withTiming(-1 * (defaultZoomScale * (e.x - viewWidth.value / 2)));
                translationY.value = withTiming(-1 * (defaultZoomScale * (e.y - viewHeight.value / 2)));
            }
        })
            .numberOfTaps(2);
        ...
        return Gesture.Race(doubleTap, simultaneousGesture
    ...
    useDerivedValue(() => {
        if (scale.value > 1 && !isZoomed.value) {
            isZoomed.value = true;
            if (zoomListContext)
                runOnJS(zoomListContext.onZoomBegin)();
            if (onZoomBegin)
                runOnJS(onZoomBegin)();
        }
            ...
            if (zoomListContext)
                runOnJS(zoomListContext.onZoomEnd)();
            if (onZoomEnd)
                runOnJS(onZoomEnd)();
        }
    ...
    const memoizedOnLayout = useCallback((e) => {
    ...
    return (React.createElement(GestureDetector, { gesture: gesture },
        React.createElement(Animated.View, { ...props, onLayout: memoizedOnLayout, style: memoizedStyle })));
}
